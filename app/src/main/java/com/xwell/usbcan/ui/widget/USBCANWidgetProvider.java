package com.xwell.usbcan.ui.widget;
...
public class USBCANWidgetProvider extends AppWidgetProvider {
	// log tag
	private static final String TAG = "USBCANWidgetProvider";
	...
	public void onReceive(Context context, Intent intent) {
		if(_streamNames.isEmpty()) {
			_streamNames.put(99, R.string.usbcan_volume_title);
		...
		final ComponentName nm = new ComponentName(context, USBCANWidgetProvider.class);
		final int[] appWidgetIds = AppWidgetManager.getInstance(context).getAppWidgetIds(nm);
		...
		if(stream == 99) {//This is the USB CAN Volume
			//Log.d(TAG, "Stream was 99! Updating CAN Volume widgets! Volume =" + intent.getIntExtra("CANUSB-Volume", 10));
			if(action.equals(context.getString(R.string.VOLUME_SET))) {
				for(int awi : appWidgetIds) {
					updateWidget(context, awm, awi, stream,intent.getIntExtra("CANUSB-Volume", 10),intent.getIntExtra("CANUSB-Mode", 6));
				}
			}
		} else { //android system volumes
	...
	public void onUpdate(Context context, AppWidgetManager appWidgetManager, int[] appWidgetIds) {
		...
		for(int awi : appWidgetIds) {
			...
			updateWidget(context, appWidgetManager, awi, stream,-1,-1);
		}
	}
	...
	static void updateWidget(Context context, AppWidgetManager appWidgetManager,
							 int appWidgetId, int stream, int specialvolume, int specialmode) {
		...
		if(stream == 99) {//Send to USBCANMonitor Service
			Intent downIntent = new Intent(context, USBCANMonitor.class);
			downIntent.setAction(context.getString(R.string.VOLUME_DOWN));
			...
			views.setOnClickPendingIntent(R.id.btndown,
									  PendingIntent.getService(context, appWidgetId, downIntent,
																 PendingIntent.FLAG_UPDATE_CURRENT));
			Intent upIntent = new Intent(context, USBCANMonitor.class);
			upIntent.setAction(context.getString(R.string.VOLUME_UP));
			...
			views.setOnClickPendingIntent(R.id.btnup,
										  PendingIntent.getService(context, appWidgetId, upIntent,
																	 PendingIntent.FLAG_UPDATE_CURRENT));
		} else {
		...
		for(int bn = 0; bn <= 15; bn += 1) {
			...
			if(bn <= max) {
				if(stream == 99) {//Send to USBCANMonitor Service
					final Intent setIntent = new Intent(context, USBCANMonitor.class);
					setIntent.setAction(context.getString(R.string.VOLUME_SET));
					...
					setIntent.putExtra(context.getString(R.string.VOL_EXTRA),(int) (bn * 2.5)); //this is scaled by 2.5 to match the trucks 0-38 volume scale
					final int request = appWidgetId << 16 | bn;
					views.setOnClickPendingIntent(id,PendingIntent.getService(context,request, setIntent,
																			PendingIntent.FLAG_UPDATE_CURRENT));
				}else {
					...
					final int request = appWidgetId << 16 | bn;
					views.setOnClickPendingIntent(id,PendingIntent.getBroadcast(context,request, setIntent,
																			PendingIntent.FLAG_UPDATE_CURRENT));
				}
			} else {
